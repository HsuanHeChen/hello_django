...
class HomePage(object):
    ...
    def go_to_my_lists_page(self):
        ...
        self.wait_for(lambda: self.test.assertEqual(self.test.browser.find_element_by_tag_name('h1').text, 'My TODO list'))
        return self
    ...
    def start_new_list(self, item_text):
        ...
        list_page.wait_for_new_item_in_list(item_text)
        return list_page
...
class ListPage(object):
    ...
    def get_list_table_rows(self):
        return self.test.browser.find_elements_by_css_selector('#id_list_table li')

    def wait_for_new_item_in_list(self, item_text):
        self.test.wait_for(lambda: self.test.assertIn(item_text, [row.text for row in self.get_list_table_rows()]))
    ...
    def add_new_item(self, item_text):
        ...
        self.get_item_input().send_keys(Keys.ENTER)
        self.wait_for_new_item_in_list(item_text)
